...
/**
 * Manages the settings panel — voice engine selection, language, API keys, model selection.
...
class KeyboardSettingsManager(
    ...
    fun refreshPanel() {
        ...
        if (voiceEngineType == VoiceEngineType.WHISPER_GROQ || voiceEngineType == VoiceEngineType.AUTO) {
            ...
            val maskedGroq = KeyboardTheme.maskKey(voiceEngine.groqApiKey)
        ...
        if (voiceEngineType == VoiceEngineType.GEMMA_API || voiceEngineType == VoiceEngineType.AUTO) {
            ...
            val maskedGemma = KeyboardTheme.maskKey(voiceEngine.gemmaApiKey)
        ...
        val closeRow = LinearLayout(context).apply {
            ...
            setOnClickListener {
                this@KeyboardSettingsManager.panel?.visibility = View.GONE
                onShowKeyboard()
            }
        }
